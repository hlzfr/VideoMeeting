...
public class H264Packetizer extends AbstractPacketizer implements Runnable {
    ...
    private void send() throws IOException, InterruptedException {
        ...
        if (streamType == 0) {
            ...
            if (!(header[0] == 0 && header[1] == 0 && header[2] == 0)) {
                ...
                L.e("NAL units are not preceeded by 0x00000001");
                streamType = 2;
                return;
            }
        } else {
        ...
        if (type == 7 || type == 8) {
            L.v("SPS or PPS present in the stream.");
            count++;
            if (count > 4) {
                sps = null;
                pps = null;
    ...
    private void resync() throws IOException {
        ...
        L.e("Packetizer out of sync ! Let's try to fix that...(NAL length: " + naluLength + ")");
        ...
        while (true) {
            ...
            if (type == 5 || type == 1) {
                ...
                if (naluLength > 0 && naluLength < 100000) {
                    oldtime = System.nanoTime();
                    L.e("A NAL unit may have been found in the bit stream !");
                    break;
                }
                if (naluLength == 0) {
                    L.e("NAL unit with NULL size found...");
                } else if (header[3] == 0xFF && header[2] == 0xFF && header[1] == 0xFF && header[0] == 0xFF) {
                    L.e( "NAL unit with 0xFFFFFFFF size found...");
                }
            }
